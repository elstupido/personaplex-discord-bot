...
This file implements the dedicated AudioEngine process. It receives raw RTP packets 
from the Discord connection, buffers them, handles jitter, and detects when a user 
has stopped speaking (Turn Finalization).

WHY A SEPARATE PROCESS?
Because Python's Global Interpreter Lock (GIL) means that if we process audio 
in the same process as the Discord websocket, heavy websocket events will cause 
audio buffering to stutter, and audio mixing will cause websocket heartbeats to drop.
"""
...
class AudioEngine(multiprocessing.Process):
    ...
    SILENCE_THRESHOLD = 0.8  # Base seconds of silence before finalizing (Lowered for responsiveness)
    ...
    def main_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                now = time.time()
                has_data = True  # We received at least one message — skip the trailing sleep
    ...
    def tick(self, now: float):
        for uid, stream in list(self.streams.items()):
            ...
            safety_buffer = min(0.5, stream.max_latency * 1.5)
            bot_lag = max(0, min(1.0, now - self.last_bot_heartbeat - 0.6)) # Cap lag penalty at 1s
