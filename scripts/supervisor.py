...
def main():
    # 0. The Citadel Protocol 🏰
    # WHY: We want a single entrypoint for the container. 
    # Passing '--run_tests' triggers the integrity suite instead of the bot.
    print(f"[supervisor] 🔍 DEBUG: sys.argv = {sys.argv}")
    
    if "--run_tests" in sys.argv or (len(sys.argv) > 1 and sys.argv[1] == "test"):
        print("[supervisor] 🏛️  INTEGRITY MODE: Entering the Testing Citadel...")
        
        citadel_path = "scripts/test_citadel.py"
        if not os.path.exists(citadel_path):
            print(f"[supervisor] 💥 ERROR: {citadel_path} not found!")
            sys.exit(1)
            
        test_proc = subprocess.run([sys.executable, citadel_path])
    ...
    if model_type == "moshi":
        ...
        server_cmd = [sys.executable, "src/server/main.py", "--host", "0.0.0.0", "--port", "10000"]
        port = 10000
    ...
    # Use the discovered host (localhost for internal, vllm-brain for external)
    ...
    while True:
        if moshi_proc and moshi_proc.poll() is not None:
