...
RUN --mount=type=cache,target=/root/.cache/pip \
    ...
    LD_LIBRARY_PATH=$TORCH_LIB_PATH:$LD_LIBRARY_PATH LD_PRELOAD=$PRELOAD_LIBS python3 -c "import os; os.environ.pop('LD_PRELOAD', None); import torchaudio; import transformer_engine; print('SUCCESS: Blackwell Wheels Verified')" && \
    rm -rf /tmp/*.whl
